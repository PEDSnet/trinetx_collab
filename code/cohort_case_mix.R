...
#' Summarize Case-Mix / Signatures for ICD10CM Conditions per VISIT
#' 
#' @param cohort a specific cohort to join to the `dx_tbl`; defaults to NULL; 
#'               if cohort is entire database, then default to NULL
#' @param dx_tbl table with condition occurrences. defaults to site CDM condition_occurrence table
...
compute_case_mix_visits <- function(cohort = NULL,
                                   dx_tbl = cdm_tbl('condition_occurrence'),
                                   vocab_tbl = vocabulary_tbl('concept'),
                                   check_string = 'mix'){
  ...
  icd_map <- dx_tbl_cohort %>%
    select(site, visit_occurrence_id, condition_source_concept_id, condition_source_value) %>%
    inner_join(icd10cm, by = c('condition_source_concept_id' = 'concept_id')) #%>%
  #compute_new()
  
  ## Site Total Counts
  total_visits_site <- icd_map %>% group_by(site) %>%
    summarise(total_row_site = n(),
              total_visits_site = n_distinct(visit_occurrence_id)) %>%
    collect()
  ...
  summary_site <- icd_cats %>%
    group_by(site, icd_header) %>%
    summarise(n_visits_site = n_distinct(visit_occurrence_id),
              n_row_site = n()) %>%
    collect()
  
  ## Final Summary Table
  final_tbl <- summary_site %>%
    left_join(total_visits_site) %>%
    mutate(prop_row_site = round(as.numeric(n_row_site) / as.numeric(total_row_site), 2),
           prop_visit_site = round(as.numeric(n_visits_site) / as.numeric(total_visits_site), 2)) %>%
    collect_new()
